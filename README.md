...
- **官方同步**：棋子白板属性与技能名对齐官方 S19；棋子归属与羁绊名称也同步官方
  （种族 + 职业）。**官方同名羁绊的效果未实装**——战斗无羁绊加成，仅保留名称、
  归属与人数档位作为展示与运营参考（生成工具 `tools/fetch_official_traits.py`、
  `tools/build_official_traits.py`、`tools/sync_official_stats.py`）。
...
`tests/test_stats_model.py` 之类回归测试。
...
│  ├─ models.py / events.py#   战斗模型（星级倍率 / 攻速档位 / AS_CAP 常量）与事件定义
...
│  ├─ units.json           #   64 枚棋子：费用、白板属性、官方羁绊 id、技能
│  ├─ traits.json          #   34 个官方羁绊：名称/档位/描述（效果未实装）
