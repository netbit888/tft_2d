...
- **64 枚棋子 / 34 个官方羁绊 / 8 件散件 → 36 件成装 + 4 件神器**：数值全部放在
...
- **官方数据全量同步**：
  ...
  - **装备**（8 散件 / 36 成装）的**名称、基础数值、特效**全部对齐官方
    `equip.js`（金铲铲 mode18），一键生成：`tools/sync_official_items.py`
    （按合成配方匹配官方 39 件成型装备，官方特效原文写入 `items.json` 的 `desc`）。
...
装备共三类：**8 件散件 → 36 件成装（两两合成）+ 4 件神器（自选台独立页，不进合成）**。
...
> 官方机制在本战斗系统内无对应系统的少数几件（秘法手套"每回合随机装备"、冠冕"队伍规模"）
> 标为 `none`，不做战斗结算。
...
- `tools/sync_official_items.py`：按合成配方把 8 散件 / 36 成装的名称、基础数值、
...
│  ├─ items.json           #   8 件散件 + 36 件成装 + 4 件神器（名称/数值/effect/官方 desc）
│  ├─ pool.json            #   卡池构成
