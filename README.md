...
- **羁绊图标**：`assets/traits/<羁绊id>.png`（官方 96x96 六边形图标，`python tools/fetch_trait_icons.py` 一键下载；缺图时程序化六边形回退）
...
- **一键获取**：贴图按体积 / 版权考虑不纳入版本库（`.gitignore` 排除了 `assets/items`
  下的图片，仅 `assets/units`、`assets/traits` 例外），拉取仓库后需先运行
...
└─ tools/
   ...
   ├─ fetch_trait_icons.py # 下载官方羁绊图标 → assets/traits/（缺图则界面程序化回退）
